...
// Pure CSS/SVG horizontal SHAP waterfall — no chart library.
// Positive (increases risk) -> red bars extending right of centre.
// Negative (reduces risk)   -> green bars extending left of centre.
// Feature names on the left, SHAP values at the bar ends in JetBrains Mono.
...
export default function ShapWaterfall({ features }: { features: ShapFeature[] }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {features.map((f) => {
        const pct = (Math.abs(f.shap_value) / maxAbs) * 50; // half-width max
        const inc = f.direction === "increases_risk";
        const color = inc ? "#FF3B30" : "#34C759";
        return (
          <div key={f.raw_feature} className="grid grid-cols-[180px_1fr_64px] items-center gap-2">
            ...
            <div className="relative h-5 bg-rowAlt rounded-sm">
              {/* centre axis */}
      ...
      <div className="mt-2 flex items-center gap-4 text-[11px] text-textMuted">
        <span className="flex items-center gap-1">
          <span className="inline-block h-2 w-3 rounded-sm" style={{ background: "#FF3B30" }} />
          increases risk
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block h-2 w-3 rounded-sm" style={{ background: "#34C759" }} />
          reduces risk
        </span>
      </div>
    </div>
